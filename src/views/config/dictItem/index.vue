<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true">
      ...
      <el-form-item>
        <el-button type="query" icon="el-icon-search" size="mini" @click="handleQuery" v-hasPermi="['config:dictItem:query']">搜索</el-button>
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['config:dictItem:export']"
        >导出</el-button>
      </el-col>
    </el-row>
    ...
    <el-table v-loading="loading" :data="dictItemList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="代码" align="center" min-width="160px" :show-overflow-tooltip="true" >
        <template slot-scope="{row}">
          <span>{{ row.code }}</span>
        </template>
      </el-table-column>
      <el-table-column label="显示顺序" align="center" width="80">
        <template slot-scope="{row}">
...
<script>
import {fetchList,getDictItem,createDictItem,updateDictItem,deleteDictItem} from "@/api/config/dictItem";
...
export default {
  ...
  created() {
    this.handleReceiveData(this.dictGroupData);
  },
  //监听父层带过来的账户信息值
  watch:{
    dictGroupData(newVal,oldVal){
      this.dictGroupData = newVal;
      this.handleReceiveData(this.dictGroupData);
    }
  },
  methods: {
    ...
    handleReceiveData(data){
      this.queryParams.groupId=this.dictGroupData.groupId;
      this.getList();
    },
    /** 查询列表 */
    getList() {
    ...
    handleUpdate(row) {
      ...
      getDictItem(id).then(response => {
        this.form = response;
        this.form.groupId = response.group.id;
        this.open = true;
        this.title = "修改";
      });
    },
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != undefined) {
            ...
            createDictItem(this.form).then(response => {
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有购买来源数据项?', "警告", {
          ...
          return exportDictItem(queryParams);
        }).then(response => {
